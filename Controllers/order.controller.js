...
const order = { 
    booking: async (req, res) => {
        ...
        const queryQueueOrder = `
          INSERT INTO QueueBookings (id, customer_id, book_date, rent_duration, discount_applied, queue_status, address) 
          VALUES (?, ?, ?, ?, ?, ?, ?);
        `;
        const valueQueueOrder = [orderID, customerID, new Date(), rent_duration, discount_applied, 'WAITING', address];
        ...
        try {
          ...
          for (let i = 0; i < gameList.length; i++) {
            ...
            INSERT INTO BookingItems (id, game_id, booking_id, price)
            VALUES ('${uuidv4()}', '${gameID}', '${orderID}', (SELECT price FROM Games WHERE id = '${gameID}'));
            `;
          ...
          const [getTotal] = await connection.query(`SELECT SUM(price) AS total_price FROM BookingItems WHERE booking_id = '${orderID}'`);

          // INSERT FINAL PRICE 
    
          const finalPrice = discount_applied ? getTotal[0].total_price * discount_applied : getTotal[0].total_price
      
          const queryInsert = `UPDATE QueueBookings SET rental_price = (${finalPrice}) WHERE id = '${orderID}'`;
    ...
    accept: async (req, res) => {
      ...
      try {
        ...
        VALUES ('${uuidv4()}}', '${userID}', 'Accept Order', 'Accept Order ID ${orderId}')`;
    ...
    reject: async (req, res) => { 
      ...
      try {
        ...
        const queryTrack = `
        INSERT INTO history_action_track (id, admin_id, event_name, action ) 
        VALUES ('${uuidv4()}', '${userID}', 'Reject Order', 'Reject Order ID ${orderId}')`;
